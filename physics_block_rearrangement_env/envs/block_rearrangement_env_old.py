...
class PhysicsBlockRearrangementEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, use_gui=False, num_blocks=3, num_locations=5, robot_type='ur3e'):
        ...
        self.tcp_offset_pos = [0, 0, 0.22]
        self.tcp_offset_orn = [0, 0, 0, 1]
        # Pre-calculate the inverse transform for convenience
        ...
        self.gripper_open_value = 0.2 # Example: Max joint value for open
        self.gripper_closed_value = 0.695  # Example: Min joint value for closed
        ...
        # Actions: Move_Above_Block(0..N-1), Move_Above_Location(0..M-1), Lower_Grasp, Raise, Release
        self.num_actions = self.num_blocks + self.num_locations + 3
        ...
        # Loop through all joints in the loaded URDF
        for i in range(num_joints):
            ...
            # Check if it's a movable arm joint (revolute and has limits defined)
            # Exclude fixed joints and joints without limits (like potentially some gripper mimic joints)
            # Assuming the first 6 valid joints found are the arm for UR3e/Panda
            if (joint_type == p.JOINT_REVOLUTE or joint_type == p.JOINT_PRISMATIC) and (lower_limit < upper_limit):
                # Check if it's likely an arm joint (e.g., based on index or name pattern if needed)
                # Simple assumption for UR/Panda: first 6 movable joints are the arm
                if len(self.arm_joint_indices) < 6:
                    ...
                    # Calculate the midpoint as a default rest pose
                    self.joint_rest_poses.append((lower_limit + upper_limit) / 2.0)
                    print(
                        f"  -> Added Arm Joint: Index={joint_index}, Name='{joint_name}', Limits=[{lower_limit:.2f}, {upper_limit:.2f}]")

        neutral_pose_angles = [0.0, -np.pi / 2, np.pi / 2, -np.pi / 2, -np.pi / 2, 0.0]
        ...
        for i in range(num_joints):
            ...
            if info[2] == p.JOINT_REVOLUTE and len(self.arm_joint_indices) < 6:  # Basic check for UR arms
    ...
    @staticmethod
    def _get_neutral_joint_angles():
        # EXAMPLE neutral pose (elbow up, pointing down) - TUNE THIS!
        # Order should match your self.arm_joint_indices
    ...
    def reset(self, seed=None, options=None):
        ...
        if len(neutral_angles) == len(self.arm_joint_indices):
            ...
            print("Warning: Mismatch between neutral angles and arm joints found.")
    ...
    def _execute_primitive(self, action_index):
        ...
        try:
            # --- Action Type: Move Above BLOCK ---
            if action_index < self.num_blocks:
                block_idx_to_target = action_index
                if block_idx_to_target >= len(self.block_ids):
                    print(f"Warning: Action targets invalid block index {block_idx_to_target}")
                    return False  # Invalid action if block doesn't exist

                block_id = self.block_ids[block_idx_to_target]
                try:
                    block_pos, _ = p.getBasePositionAndOrientation(block_id, physicsClientId=self.client)
                except Exception as e:
                    print(f"Warning: Could not get pose for block ID {block_id}. Maybe removed? {e}")
                    return False  # Cannot target block if it doesn't exist

                # Calculate target TCP pose above the block's current position
                cube_scale = 0.05  # Assuming 5cm cube
                half_block_height = cube_scale / 2.0
                target_tcp_pos = [block_pos[0], block_pos[1],
                                  block_pos[2] + half_block_height + 0.10]  # 10cm above block top

                tool0_target_pos, tool0_target_orn = self._get_tool0_pose_from_tcp_pose(target_tcp_pos, target_ori_ee)
                success = self._move_ee_to_pose(tool0_target_pos, tool0_target_orn)

            # --- Action Type: Move Above LOCATION ---
            elif action_index < self.num_blocks + self.num_locations:
                loc_idx = action_index - self.num_blocks  # Adjust index
                if loc_idx >= len(self.fixed_locations):
                    print(f"Warning: Action targets invalid location index {loc_idx}")
                    return False

                target_pos_table = self.fixed_locations[loc_idx]
                # Target TCP pose above the fixed location Z
                target_tcp_pos = [target_pos_table[0], target_pos_table[1],
                                  target_pos_table[2] + 0.15]  # e.g., 15cm above table at location XY

                tool0_target_pos, tool0_target_orn = self._get_tool0_pose_from_tcp_pose(target_tcp_pos, target_ori_ee)
                success = self._move_ee_to_pose(tool0_target_pos, tool0_target_orn)

            # --- Action Type: Lower and Grasp ---
            elif action_index == self.num_blocks + self.num_locations:
                # --- Your existing Grasp logic here ---
                # (Checks if holding, moves down from CURRENT pose, closes, checks object, constrains)
                # ... (Make sure it uses current EE pose for lowering target) ...
                if self.held_object_id is not None: return False
                current_tool0_pos, current_tool0_orn = self._get_tool0_pose()
                if current_tool0_pos is None: return False

                target_tcp_pos_down = [current_tool0_pos[0], current_tool0_pos[1],
                                       self.table_height + 0.03]  # Use current XY, target low Z for TCP
                tool0_target_pos_down, tool0_target_orn_down = self._get_tool0_pose_from_tcp_pose(target_tcp_pos_down,
                                                                                                  target_ori_ee)  # Target orientation down

                if not self._move_ee_to_pose(tool0_target_pos_down, tool0_target_orn_down, max_steps=50): return False
                self._wait_steps(20)
                if not self._set_gripper(open_gripper=False): return False
                self._wait_steps(50)
                grasped_obj_id = self._get_object_in_gripper(check_dist=0.04)
                if grasped_obj_id is not None:
                    self.held_object_id = grasped_obj_id
                    # Add constraint logic (ensure rel_pos/rel_ori are correct)
                    # ... (constraint code from before) ...
                    success = True  # Assume constraint creation works for now
                else:
                    self._set_gripper(open_gripper=True)
                    success = False

            # --- Action Type: Raise Gripper ---
            elif action_index == self.num_blocks + self.num_locations + 1:
                # --- Your existing or modified Raise logic here ---
                # (Gets current TCP, targets a higher Z, transforms, moves)
                current_tcp_pos, current_tcp_orn = self._get_tcp_pose()
                if current_tcp_pos is None: return False
                # Use fixed raise height above table for consistency
                safe_raise_height = self.table_height + 0.10  # Example: 15cm above table
                target_tcp_pos = [current_tcp_pos[0], current_tcp_pos[1], safe_raise_height]
                tool0_target_pos, tool0_target_orn = self._get_tool0_pose_from_tcp_pose(target_tcp_pos,
                                                                                        current_tcp_orn)  # Keep current orientation
                success = self._move_ee_to_pose(tool0_target_pos, tool0_target_orn)

            # --- Action Type: Release ---
            elif action_index == self.num_blocks + self.num_locations + 2:
                # --- Your existing Release logic here ---
                # (Checks if holding, removes constraint, opens gripper)
                if self.held_object_id is None: return False
                if self.grasp_constraint_id is not None:
                    try:
                        p.removeConstraint(self.grasp_constraint_id, physicsClientId=self.client)
                    except Exception as e:
                        pass
                    self.grasp_constraint_id = None
                gripper_opened = self._set_gripper(open_gripper=True)
                self.held_object_id = None
                self._wait_steps(50)
                success = gripper_opened
    ...
    def _set_gripper(self, open_gripper):
        ...
        try:
            ...
            print("Joint states before after: ",
                  p.getJointStates(self.robot_id, [2, 3, 4, 5, 6, 7, 12, 14, 16, 17, 19, 21]))
            self._wait_steps(50)
            return True
        except Exception as e:
